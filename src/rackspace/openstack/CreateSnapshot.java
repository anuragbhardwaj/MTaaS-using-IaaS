...
public class CreateSnapshot implements Closeable {
   ...
   public void createSnapshot(String vmId, String vmName) {
	...
	// Authentication in jclouds is lazy and happens on the first call to the cloud.
	   ...
	   serverApi.createImageFromServer(vmName + "_snapshot",vmId);
	   forImageID();
	   //System.out.println("Image ID: " + images.get(0).getId().toString());
	   
   }
   
   public void forImageID(){
	   ...
	   NovaApi novaApi = ContextBuilder.newBuilder(provider)
		   ...
		   ServerApi serverApi = novaApi.getServerApiForZone(""+ novaApi.getConfiguredZones().iterator().next());
		   System.out.println("Image ID: " + images.get(0).getId().toString());
		   //tf.startFTP("cmpe283_test2_snapshot");
   }
