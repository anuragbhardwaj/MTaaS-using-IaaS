        ...
        <tbody>
            ...
            <c:forEach var="Vm" items="${VmDatas}" >
                <tr>
                    ...
                    <c:choose>
    					<c:when test="${Vm.instance_Taken == '0'}">
    					
    					</c:when>
    					<c:otherwise>
      						<a href="VmController?action=stop&vmId=<c:out value="${Vm.instance_ID}"/>">Release</a>
    					</c:otherwise>
 					</c:choose>
 					</td>
 					<td>
 					<c:choose>
    					<c:when test="${Vm.instanceStatus == '0'}">
